...
class CalculateTrajectories():
    ...
    def initializeGraph(self, A, connect4):
        for i in range(2*self.rows):
            for j in range(2*self.columns):
                if A[i, j]:
                    if i > 0 and A[i-1][j]:
                        self.AddToAllEdges(i*2*self.columns+j, (i-1)*2*self.columns+j, 1)
                    if i < 2*self.rows-1 and A[i+1][j]:
                        self.AddToAllEdges(i*2*self.columns+j, (i+1)*2*self.columns+j, 1)
                    if j > 0 and A[i][j-1]:
                        self.AddToAllEdges(i*2*self.columns+j, i*2*self.columns+j-1, 1)
                    if j < 2*self.columns-1 and A[i][j+1]:
                        self.AddToAllEdges(i*2*self.columns+j, i*2*self.columns+j+1, 1)

                    if not connect4:
                        if i > 0 and j > 0 and A[i-1][j-1]:
                            self.AddToAllEdges(i*2*self.columns+j, (i-1)*2*self.columns+j-1, 1)
                        if i < 2*self.rows-1 and j < 2*self.columns-1 and A[i+1][j+1]:
                            self.AddToAllEdges(i*2*self.columns+j, (i+1)*2*self.columns+j+1, 1)
                        if i > 2*self.rows-1 and j > 0 and A[i+1][j-1]:
                            self.AddToAllEdges(i*2*self.columns+j, (i+1)*2*self.columns+j-1, 1)
                        if i > 0 and j < 2*self.columns-1 and A[i-1][j+1]:
                            self.AddToAllEdges(i*2*self.columns+j, (i-1)*2*self.columns+j+1, 1)
    ...
    def RemoveTheAppropriateEdges(self):
        for i in range(self.MSTedges):
            ...
            if np.absolute(e.src - e.dst) == 1:
                alpha = (4*minN+3) - 2*(maxN % self.columns)
                eToRemove = Edge(alpha, alpha+2*self.columns, 1)
                eToRemoveMirr = Edge(alpha+2*self.columns, alpha, 1)
                eToRemove2 = Edge(alpha+1, alpha+1+2*self.columns, 1)
                eToRemove2Mirr = Edge(alpha+1+2*self.columns, alpha+1, 1)

            else:
                alpha = (4*minN+2*self.columns) - 2*(maxN % self.columns)
                eToRemove = Edge(alpha, alpha+1, 1)
                eToRemoveMirr = Edge(alpha+1, alpha, 1)
                eToRemove2 = Edge(alpha+2*self.columns, alpha+1+2*self.columns, 1)
                eToRemove2Mirr = Edge(alpha+1+2*self.columns, alpha+2*self.columns, 1)
    ...
    def CalculatePathsSequence(self, StartingNode):
        ...
        movement.append(2*self.columns)
        movement.append(-1)
        movement.append(-2*self.columns)
        movement.append(1)
        ...
        while True:
            ...
            i = int(currentNode/(2*self.columns))
            j = currentNode % (2*self.columns)
            previ = int(prevNode/(2*self.columns))
            prevj = prevNode % (2*self.columns)
