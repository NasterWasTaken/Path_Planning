...
from CalcTrajectories import CalculateTrajectories
...
def get_area_map(path, area=0, obs=-1):
    ...
    le_map[ma] = area # type: ignore
    le_map[~ma] = obs # type: ignore
    return le_map
...
def get_area_indices(area, value, inv=False, obstacle=-1):
    ...
    try:
        ...
        mask = area == value[0] # type: ignore
        if inv:
            mask = area != value[0] # type: ignore
        for v in value[1:]: # type: ignore
            if inv:
                mask &= area != v
            else:
                mask |= area == v
...
class MultiRobotPathPlanner(DARP):
    def __init__(self, nx, ny, notEqualPortions, initial_positions, portions,
        ...
        if not self.DARP_success:
            ...
            for mode in range(4):
                MSTs = self.calculateMSTs(self.darp_instance.BinaryRobotRegions, self.darp_instance.droneNum, self.darp_instance.rows, self.darp_instance.columns, mode)
                AllRealPaths = []
                for r in range(self.darp_instance.droneNum):
                    ct = CalculateTrajectories(self.darp_instance.rows, self.darp_instance.columns, MSTs[r])
                    ct.initializeGraph(self.CalcRealBinaryReg(self.darp_instance.BinaryRobotRegions[r], self.darp_instance.rows, self.darp_instance.columns), True)
                    ct.RemoveTheAppropriateEdges()
                    ct.CalculatePathsSequence(4 * self.darp_instance.initial_positions[r][0] * self.darp_instance.columns + 2 * self.darp_instance.initial_positions[r][1])
                    AllRealPaths.append(ct.PathSequence)

                self.TypesOfLines = np.zeros((self.darp_instance.rows*2, self.darp_instance.columns*2, 2))
                for r in range(self.darp_instance.droneNum):
                    flag = False
                ...
                subCellsAssignment = np.zeros((2*self.darp_instance.rows, 2*self.darp_instance.columns))
                for i in range(self.darp_instance.rows):
                    for j in range(self.darp_instance.columns):
            ...
            #                                 self.darp_instance.droneNum, self.darp_instance.color)
    ...
    def CalcRealBinaryReg(self, BinaryRobotRegion, rows, columns):
        temp = np.zeros((2*rows, 2*columns))
        RealBinaryRobotRegion = np.zeros((2 * rows, 2 * columns), dtype=bool)
    ...
    def calculateMSTs(self, BinaryRobotRegions, droneNum, rows, columns, mode):
        MSTs = []
        for r in range(droneNum):
            k = Kruskal(rows, columns)
