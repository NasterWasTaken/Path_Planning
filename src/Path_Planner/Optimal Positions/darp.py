...
@njit(fastmath=True)
def assign(droneNum, rows, columns, GridEnv, MetricMatrix, A):

    ArrayOfElements = np.zeros(droneNum)
    for i in range(rows):
        for j in range(columns):
            if GridEnv[i, j] == -1:
                minV = MetricMatrix[0, i, j]
                indMin = 0
                for r in range(droneNum):
...
@njit(fastmath=True)
def euclidian_distance_points2d(array1: np.array, array2: np.array) -> np.float_: # type: ignore
...
@njit(fastmath=True)
def constructBinaryImages(labels_im, robo_start_point, rows, columns):
...
@njit(fastmath=True)
def CalcConnectedMultiplier(rows, columns, dist1, dist2, CCvariation):
    returnM = np.zeros((rows, columns))
    MaxV = 0
    MinV = 2**30
...
class DARP:
    def __init__(self, nx, ny, notEqualPortions, given_initial_positions, given_portions, obstacles_positions,
        ...
        self.rows = nx
        self.columns = ny
        self.initial_positions, self.obstacles_positions, self.portions = self.spark_farm(given_initial_positions, given_portions, obstacles_positions, notEqualPortions)
        ...
        self.droneNum = len(self.initial_positions)
        self.A = np.zeros((self.rows, self.columns))
        self.GridEnv = self.defineGridEnv()
   
        self.connectivity = np.zeros((self.droneNum, self.rows, self.columns), dtype=np.uint8)
        self.BinaryRobotRegions = np.zeros((self.droneNum, self.rows, self.columns), dtype=bool)
        ...
        self.ArrayOfElements = np.zeros(self.droneNum)
        self.color = []

        for r in range(self.droneNum):
            np.random.seed(r)
            self.color.append(list(np.random.choice(range(256), size=3)))
        
        np.random.seed(1)
        if self.visualization:
            self.assignment_matrix_visualization = darp_area_visualization(self.A, self.droneNum, self.color, self.initial_positions)

    def spark_farm(self, given_initial_positions, given_portions, obs_pos, notEqualPortions):
        initial_positions = []
        for position in given_initial_positions:
            if position < 0 or position >= self.rows * self.columns:
                print("Initial positions should be inside the Grid.")
                sys.exit(1)
            initial_positions.append((position // self.columns, position % self.columns))

        obstacles_positions = []
        for obstacle in obs_pos:
            if obstacle < 0 or obstacle >= self.rows * self.columns:
                print("Obstacles should be inside the Grid.")
                sys.exit(2)
            obstacles_positions.append((obstacle // self.columns, obstacle % self.columns))
    ...
    def defineGridEnv(self):
        GridEnv = np.full(shape=(self.rows, self.columns), fill_value=-1)  # create non obstacle map with value -1
        ...
        connectivity = np.zeros((self.rows, self.columns))
    ...
    def divideRegions(self):
        success = False
        cancelled = False
        criterionMatrix = np.zeros((self.rows, self.columns))
        iteration = 0

        while self.termThr <= self.dcells and not success and not cancelled:
            downThres = (self.Notiles - self.termThr*(self.droneNum-1))/(self.Notiles*self.droneNum)
            upperThres = (self.Notiles + self.termThr)/(self.Notiles*self.droneNum)
            ...
            while iteration <= self.MaxIter and not cancelled:
                self.A, self.ArrayOfElements = assign(self.droneNum,
                                                      self.rows,
                                                      self.columns,
                                                      self.GridEnv,
                                                      self.MetricMatrix,
                                                      self.A)
                ConnectedMultiplierList = np.ones((self.droneNum, self.rows, self.columns))
                ConnectedRobotRegions = np.zeros(self.droneNum)
                plainErrors = np.zeros((self.droneNum))
                divFairError = np.zeros((self.droneNum))

                self.update_connectivity()
                for r in range(self.droneNum):
                    ConnectedMultiplier = np.ones((self.rows, self.columns))
                    ...
                    if num_labels > 2:
                        ...
                        BinaryRobot, BinaryNonRobot = constructBinaryImages(labels_im, self.initial_positions[r], self.rows, self.columns)
                        ConnectedMultiplier = CalcConnectedMultiplier(self.rows, self.columns,
                    ...
                    plainErrors[r] = self.ArrayOfElements[r]/(self.DesireableAssign[r]*self.droneNum)
                ...
                correctionMult = np.zeros(self.droneNum)
    ...
    def FinalUpdateOnMetricMatrix(self, CM, RM, currentOne, CC):
        MMnew = np.zeros((self.rows, self.columns))
    ...
    def construct_Assignment_Matrix(self):
        Notiles = self.rows*self.columns
        fair_division = 1/self.droneNum
        effectiveSize = Notiles - self.droneNum - len(self.obstacles_positions)
        termThr = 0

        if effectiveSize % self.droneNum != 0:
            termThr = 1

        DesireableAssign = np.zeros(self.droneNum)
        MaximunDist = np.zeros(self.droneNum)
        MaximumImportance = np.zeros(self.droneNum)
        MinimumImportance = np.zeros(self.droneNum)
        ...
        AllDistances = np.zeros((self.droneNum, self.rows, self.columns))
        TilesImportance = np.zeros((self.droneNum, self.rows, self.columns))
    ...
    def calculateCriterionMatrix(self, TilesImportance, MinimumImportance, MaximumImportance, correctionMult, smallerthan_zero,):
        returnCrit = np.zeros((self.rows, self.columns))
        if self.importance:
            if smallerthan_zero:
