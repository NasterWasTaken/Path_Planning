...
class Kruskal(object):
    def __init__(self, rows, columns):
        ...
        self.allEdges = []
        self.MAX_NODES = self.rows*self.columns
        self.nodes = {}
            ...
            for j in range(self.columns):
                if (A[i][j]):
                    if (mode == 0):
                        ...
                        cost1 = self.columns - j
                    elif mode == 3:
                        cost1 = j+1

                    if (i > 0 and A[i-1][j]):
                        self.AddToAllEdges(i*self.columns+j, (i-1)*self.columns+j, cost1)
                    if (i < self.rows-1 and A[i+1][j]):
                        self.AddToAllEdges(i*self.columns+j, (i+1)*self.columns+j, cost1)
                    if (j > 0 and A[i][j-1]):
                        self.AddToAllEdges(i*self.columns+j, i*self.columns+j-1, cost2)
                    if (j < self.columns-1 and A[i][j+1]):
                        self.AddToAllEdges(i*self.columns+j, i*self.columns+j+1, cost2)

                    if not connect4:
                        if (i > 0 and j > 0 and A[i-1][j-1]):
                            self.AddToAllEdges(i*self.columns+j, (i-1)*self.columns+j-1, 1)
                        if (i < self.rows-1 and j < self.columns-1 and A[i+1][j+1]):
                            self.AddToAllEdges(i*self.columns+j, (i+1)*self.columns+j+1, 1)
                        if (i > self.rows-1 and j > 0 and A[i+1][j-1]):
                            self.AddToAllEdges(i*self.columns+j, (i+1)*self.columns+j-1, 1)
                        if (i > 0 and j < self.columns-1 and A[i-1][j+1]):
                            self.AddToAllEdges(i*self.columns+j, (i-1)*self.columns+j+1, 1)
